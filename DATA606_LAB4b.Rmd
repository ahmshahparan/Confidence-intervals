...
mean(population)
```
The distribution seems to be right skewed. I would not consider this is a “typical” sample size if we compare it to our original population mean of 1499.69. A “typical” mean is the value that is near the most concurrent values in a distribution.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

No, it won't be beacuse this is a random sample of 60 randomly selected observations. I expect another student’s distribution to be similar to but not same as mine.
...
The sample observations are random, sample size is greater than 30 and population distribution is not strongly skewed.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

 95% confidence means that roughly 95% of the time the estimates will be within 2 standard errors of the parameter.
...
mean(population)
```
Based on the 95% confidence interval calculation vs. the population mean, yes, the interval does capture the true average size of houses in Ames.
...
I expect 95% of the confidence intervals to capture true population mean. As the intervals are built for 95% confidence level, I expect 95% of the intervals to capture true population mean
...
1)   Using the following function (which was downloaded with the data set), 
    ...
    ```{r}
    p <- 1-(3/50)
    #Proportion of confidence intervals plot:
    p
    ```
94% of the confidence intervals include true population mean. This is very close to the confidence level selected but not exactly same. The confidence level is a good approximate measure but not a perfect calculation

2)   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
```{r}
#Significance level (α) at 90% confidence level
cl <- 90
alpha <- 1-(cl/100)
cp <- 1-(alpha/2) 
cp
...
```
```{r}
#Proportion of CI plot at 90% CI
p <- 1-(4/50)
p
```
Visually, I can see that 82% of intervals include the true population mean. It is very close to the confidence level selected.
